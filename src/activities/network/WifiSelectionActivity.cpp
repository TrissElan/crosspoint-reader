...
void WifiSelectionActivity::renderNetworkList() const {
  ...
  if (networks.empty()) {
    ...
    renderer.drawCenteredText(UI_12_FONT_ID, top, tr(STR_NO_NETWORKS));
...
void WifiSelectionActivity::renderConnecting() const {
  ...
  if (state == WifiSelectionState::SCANNING) {
    renderer.drawCenteredText(UI_12_FONT_ID, top, tr(STR_SCANNING));
  } else {
    ...
    renderer.drawCenteredText(UI_12_FONT_ID, top, ssidInfo.c_str());
  }
}
...
void WifiSelectionActivity::renderConnected() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, top + 10, ssidInfo.c_str());

  const std::string ipInfo = std::string(tr(STR_IP_ADDRESS_PREFIX)) + connectedIP;
  renderer.drawCenteredText(UI_12_FONT_ID, top + 40, ipInfo.c_str());
...
void WifiSelectionActivity::renderSavePrompt() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, top + 40, tr(STR_SAVE_PASSWORD));
  ...
  if (savePromptSelection == 0) {
    ...
    renderer.drawText(UI_12_FONT_ID, startX, buttonY, text.c_str());
  } else {
    renderer.drawText(UI_12_FONT_ID, startX + 4, buttonY, tr(STR_YES));
  }

  // Draw "No" button
  if (savePromptSelection == 1) {
    std::string text = "[" + std::string(tr(STR_NO)) + "]";
    renderer.drawText(UI_12_FONT_ID, startX + buttonWidth + buttonSpacing, buttonY, text.c_str());
  } else {
    renderer.drawText(UI_12_FONT_ID, startX + buttonWidth + buttonSpacing + 4, buttonY, tr(STR_NO));
  }
...
void WifiSelectionActivity::renderConnectionFailed() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, top + 20, connectionError.c_str());
...
void WifiSelectionActivity::renderForgetPrompt() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, top + 40, tr(STR_FORGET_AND_REMOVE));
  ...
  if (forgetPromptSelection == 0) {
    ...
    renderer.drawText(UI_12_FONT_ID, startX + 4, buttonY, tr(STR_CANCEL));
  }
  ...
  if (forgetPromptSelection == 1) {
    ...
    renderer.drawText(UI_12_FONT_ID, startX + buttonWidth + buttonSpacing + 4, buttonY, tr(STR_FORGET_BUTTON));
  }
